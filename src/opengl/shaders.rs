pub const RT_COMPUTE_SRC: &str = r#"
    ...
    layout (binding = 1) uniform sampler2D uEnvMap;
    ...
    vec3 skyColor(vec3 rd) {
        vec3 d = normalize(rd);
        vec2 uv = vec2(atan(d.z, d.x) * 0.15915494 + 0.5, 0.5 - asin(clamp(d.y, -1.0, 1.0)) * 0.31830989); // * 1/pi
    ...
    void main() {
        ...
        for (int s = 0; s < SPP; s++) {
            ...
            col += min(rayColor(uCamPos, rd, rng), vec3(8.0)); // clamp fireflies from the bright sky
        }
        col /= float(SPP);
